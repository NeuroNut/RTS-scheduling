...
#define MAX_TASKS 10
#define MAX_JOBS 100
#define QUANTUM 1  // Extension quantum to reduce context switches
...
// Generate all jobs for all tasks up to hyperperiod
void generate_jobs() {
    job_count = 0;
...
int is_extension_feasible(int current_job_idx, int current_time, int quantum) {
    ...
    // Simulate RM scheduling from current_time + extension to hyperperiod
    ...
    while (time < hyperperiod) {
        ...
        if (next_job == -1) {
            // No job ready, find next release
...
void simulate_rmrcs() {
    ...
    while (current_time < hyperperiod) {
        ...
        if (next_job_idx == -1) {
            // No job ready, advance time to next release
            int next_release = hyperperiod;
            for (int i = 0; i < job_count; i++) {
        ...
        // Check if current job different from next job and has lower priority
        ...
        // Execute current job for remaining time or until next event
        int next_release = hyperperiod;
        for (int i = 0; i < job_count; i++) {
            if (jobs[i].release > current_time && jobs[i].release < next_release) {
                next_release = jobs[i].release;
...
// Merge consecutive schedule entries for same task-job
